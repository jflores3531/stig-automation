...
- **`l2_stig_harden_logging_access.py`** — Only the logging/audit and access-control fixes, as their own script: log timestamps, buffer size, login success/failure logging, `logging userinfo`, `logging trap critical`, syslog servers, the `archive`/`log config` block that answers eight rules at once, the failed-login lockout, `exec-timeout` on vty **and** console, and the concurrent-session limit. Nothing here touches the data plane — no spanning-tree mode, no VLAN database, no `no <service>` lines — so it needs no change window for forwarding behaviour. `--dry-run` prints the commands without connecting; `--sessions N` overrides the session count. On the session limit it pushes both `session-limit` *and* DISA's primary method of taking the spare vty lines out of service with `transport input none`, and deliberately omits `ip http max-connections` — a limit on a server hardening turns off is not a control. **After it runs the switch accepts only N (default 2) concurrent SSH sessions.**
