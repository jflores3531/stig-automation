...
### Audit & hardening
- **`stig_common.py`** — Shared audit engine: loads a DISA `.cklb` checklist, checks the device against it, reports PASS/FAIL/NOT AUTOMATED by severity. Every answered rule also carries an `Inspected with: ...` line naming the commands its verdict was read from — `show running-config` for most, but `show snmp user`, `show vtp password`, `show version`, `show spanning-tree`, `show vlan brief` or `show ip ssh` for the rules that need them — in the report and in the exported checklist's box. A rule with no check claims nothing, since a command line under a rule nothing examined would misrepresent the audit's own work. Rules whose evidence lives in running-config also carry a `Verify with: \`show running-config | section ^line vty\`` line — the filtered command that shows that evidence **on the switch**, for a reviewer who would rather look than take the report's word for it. It is deliberately a separate label: the audit reads running-config once and greps in Python, so it never sends a `|` filter, and claiming one under "Inspected with" would describe a command nobody ran.
...
- **`securecrt/harden_l2s_bulk.py`** — The only script in `securecrt/` that **configures devices**. It pushes the same logging/audit and access-control fixes as `scripts/l2_stig_harden_logging_access.py` to every saved session, or to one distribution-node folder, over SecureCRT instead of netmiko — which is the whole point, since netmiko cannot be installed on the machine these switches are reachable from. Same commands, same order; the lists are duplicated because nothing in `securecrt/` may import from the repo, and `tests/test_securecrt_harden.py` asserts the two copies are identical so they cannot drift. It is a separate file with a separate name rather than a flag on one of the walks above, because a script that logs into a fleet on its own authority and writes to running-config is a materially different thing to put in front of whoever approved read-only tooling. **It never writes startup-config** — until someone runs `copy running-config startup-config`, a reload puts the switch back exactly as it was, so the order is harden, re-audit, then save deliberately. **The vty block is off unless asked for**, defaulting to No: everything else is reversible from any session that can still reach the switch, while the vty lines decide how many sessions there can be. When it is asked for, the range is read off each switch — the limit is **5**, so vty 0-4 answer and everything above them is taken out of service. IOS XE ships `line vty 0 4` *and* `line vty 5 15`, and configuring only the first leaves eleven lines answering while the row claims a five-session limit; on a switch whose only range is `line vty 0 4` there is nothing above it and what gets pushed is DISA's own example verbatim — and a switch whose range could not be read has the block **skipped and flagged**, never applied on an assumed `0 4`. The syslog collectors are read straight out of `inventory.yaml` when the folder is still inside the repo — it is JSON, so that needs no import from the wider project — and **the run does not ask for them at all** in that case; the confirmation still lists the `logging host` lines, so nothing goes out unseen. Where the folder has been copied away, or the file lists fewer than two usable addresses, the box appears and names the exact path it looked at and what it found there, so an empty box distinguishes a wrong folder from wrong contents. Two or none either way, since a switch logging to one server is a partial fix for V-220568/220620 that reads as a finished one. The SSH configuration is **read back off the switch** after the push — `show ip ssh` for the version, `show running-config | include ^ip ssh` for the algorithm lines rather than assumed from the config block's output: a line the parser accepted is not the same as a line in running-config, an image without `aes256-gcm` rejects that whole line and keeps what it had, and a Catalyst 9300 or 3850 never renders `ip ssh version 2` at all: SSHv1 is gone on those trains, so v2-only is not a setting and `show ip ssh` is the only place that says `SSH Enabled - version 2.0`. Version 1.99 is not counted as v2 — that is compatibility mode, where the switch still answers SSHv1. Anything missing is named in the row and the outcome says `ssh crypto incomplete`. Each run writes a `harden_log_<stamp>.csv` accounting for every session — `hostname, ip_address, outcome, rejected, ssh, comment, session, timestamp` — so a switch that was unreachable, or that rejected a command its image does not have, is a row rather than a gap. The scope prompt is a prefix match on the session path, so typing a session's full name scopes the run to that one switch — how you test it on a lab switch first — and the confirmation box **names** the sessions it matched, not just their count, because a partial filter takes in neighbours (`10.1.2.3` also matches `10.1.2.30`). A `STOP` file in the log folder ends the run cleanly at the end of the current switch. Needs `capture_l2s.py` and `capture_l2s_bulk.py` beside it.
- **`securecrt/harden_access_ports_bulk.py`** — The access-port half of the interface hardening, run from SecureCRT instead of netmiko: `switchport mode access`, PortFast, UUFB, storm control on **both** broadcast and unicast at `level 5.00` on every host-facing port of every saved session, plus `switchport access vlan <unused>` on the ports already shut (V-220641). **It never sends a trunk command and never enters a trunk-classified interface** — trunk ports are their own script and their own change window. Before classifying anything it reads each sourced interface template off the switch (`show template interface source user <name>`) and expands it, because a port whose block is only `source template UPLINK` carries no `switchport mode trunk` line of its own: off the raw config that uplink is an access port, and this script would send it `switchport mode access` and PortFast — collapsing the trunk and putting PortFast on a port that receives BPDUs as a matter of course. A switch whose templates cannot be read is skipped and logged rather than guessed at. The unused VLAN comes from `inventory.yaml` when the repo is beside the folder, and is prompted for when it is not. Writes `access_ports_log_<stamp>.csv` with each switch's access/trunk/shut port counts, so a switch classified oddly is visible without opening it. Never writes startup-config. V-220642 (default access VLAN on live ports) and V-220623 (802.1x/MAB) stay findings, and the run says so. Needs `capture_l2s.py`, `capture_l2s_bulk.py` and `harden_l2s_bulk.py` beside it — it takes session discovery and connect handling from the second and the config-push machinery from the third.
...
- **`l2_stig_harden_access_ports.py`** — Host-facing port fixes: explicit access mode, PortFast (V-220630b — without it the global `bpduguard default` is inert everywhere, a false PASS), UUFB, and storm control on **both** broadcast and unicast at `level 5.00` — DISA's Note allows a percentage in place of a bps rate, and a percentage is the same threshold on every port speed, which a bps figure is not (FastEthernet is skipped, per DISA's own Fix Text note). Access ports that are **already shut** are parked on `inventory.yaml`'s `unused_vlan` (V-220641) — a shut port forwards nothing whatever VLAN it is on, which is what makes it the one VLAN assignment a bulk pass can make safely. Never sends a trunk command or touches a trunk-classified port, so it is safe to run on a working day. **It does not set the default access VLAN** (V-220642) — that one lands on live ports, where a port still on VLAN 1 has something on it that moves with it, and bulk-assigning it once moved the lab's own management port and cut the session pushing the change; it is printed as an unpushed finding instead. **Interface templates are read off the switch and expanded before any port is classified**, since a port whose block is only `source template <name>` carries no `switchport mode trunk` line of its own — off the raw config a templated uplink reads as an access port, and this script would have sent it `switchport mode access` and PortFast. **802.1x/MAB (V-220623) is deliberately not pushed** — `authentication port-control auto` on a switch with no reachable RADIUS authenticator and no supplicant on the endpoint authenticates nobody and forwards nothing, which on an access switch full of user devices is an outage rather than a hardening step. The script prints it as an unpushed finding on every run and the audit still reports it as one.
